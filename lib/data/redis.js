...
RedisDatabase.prototype = {
  ...
  init: function (callback) {
    ...
    redisClient = redis.createClient(conf.redis, { no_ready_check: true });
  ...
  // Card commands
  createCard: function (room, card, callback) {
    var prefix = REDIS_PREFIX + '-room:' + room + '-cards';
    redisClient.incr(prefix + ':id', function (err, newUniqueID) {
      card.id = newUniqueID;
      var cardString = JSON.stringify(card);
      redisClient.hset(prefix, newUniqueID, cardString);
      console.log("Creating new card " + cardString);
  ...
  addSticker: function (room, cardId, stickerId) {
    redisClient.hget(REDIS_PREFIX + '-room:' + room + '-cards', cardId, function (err, res) {
      ...
      if (null === card.sticker || "nosticker" === stickerId)
        card.sticker = [];
